...
export default function SubmitQuoteForm({ rfp }: { rfp: any }) {
  ...
  useEffect(() => {
    const loadDraft = async () => {
      try {
        console.log("🔵 FRONTEND → RFP OBJECT:", rfp);
        console.log("🔵 FRONTEND → project_id:", rfp.project_id);
        ...
        console.log("🟢 FRONTEND → DRAFT RESPONSE:", data);
        ...
        if (data && Object.keys(data).length) {
          ...
          console.log("⚠️ FRONTEND → No draft data found");
        }
...
..."
      ...
      {message && (
        <p
          className={`text-center text-lg font-medium mt-6 ${message.type === 'success'
            ? 'text-green-600'
            : 'text-red-600'
            }`}
        >
