...
import { X, DollarSign, Calendar, FileText, Upload, CheckCircle } from 'lucide-react';
...
export default function QuoteDetailsModal({ quote, isOpen, onClose }: QuoteDetailsModalProps) {
  ...
  let parsedMessage;
  try {
    parsedMessage = JSON.parse(quote.proposal_message);
  } catch (e) {
    console.error('Proposal message parse error:', e);
    parsedMessage = {};
  }

  const { technical, delivery, milestones = [], caseStudies = [], references = [], attachments = [] } = parsedMessage;
  ...
  const handleAward = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(await response.json().then((data) => data.error || 'Failed to award contract'));
      }

      console.log('Contract awarded successfully');
      onClose();
      router.refresh(); // Refresh page to update statuses
    } catch (err: any) {
