// src/components/dashboard/SubmitQuoteForm.tsx (Updated to match screenshot UI exactly - credits removed)
'use client';
...
export default function SubmitQuoteForm({ rfp }: { rfp: any }) {
  ...
  const technicalFileRef = useRef<HTMLInputElement>(null);
  const caseStudyFileRef = useRef<HTMLInputElement>(null);
  ...
  const handleFileChange = (files: FileList | null, type: 'general' | 'technical' | 'caseStudy') => {
    if (files) {
      const newFiles = Array.from(files);
      if (type === 'general') {
        setAttachments([...attachments, ...newFiles]);
      }
      // You can extend for technical/case study attachments if needed
    }
  };
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault();
    handleFileChange(e.dataTransfer.files, 'general');
  };
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      ...
      <div className="bg-white rounded-2xl shadow-md p-6 mb-8">
        <h1 className="text-2xl font-bold text-gray-900">{rfp.title || 'Untitled Project'}</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 text-sm">
          <div>
            <span className="text-gray-600">Client</span>
            <p className="font-medium text-gray-900">{rfp.buyer_name || 'Acme Corp.'}</p>
          </div>
          <div>
            <span className="text-gray-600">Deadline</span>
            <p className="font-medium text-gray-900">
              {rfp.submission_deadline ? new Date(rfp.submission_deadline).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : '2024-12-31'}
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <section className="bg-white rounded-2xl shadow-md p-6">
          ...
          <button
            type="button"
            onClick={() => technicalFileRef.current?.click()}
            className="mt-3 text-blue-600 hover:text-blue-800 flex items-center gap-2 font-medium"
          >
            <Paperclip className="w-5 h-5" /> Attach Files
          </button>
          <input ref={technicalFileRef} type="file" multiple hidden onChange={e => handleFileChange(e.target.files, 'technical')} />
        </section>
...
..."
        ...
        <section className="bg-white rounded-2xl shadow-md p-6">
          ...
          <p className="text-sm text-gray-600 mb-3">Provide links to relevant past projects or attach files.</p>
        ...
        <section className="bg-white rounded-2xl shadow-md p-6">
          ...
          >
            ...
            <input
              ...
              multiple
              hidden
              onChange={e => handleFileChange(e.target.files, 'general')}
            />
          </div>
